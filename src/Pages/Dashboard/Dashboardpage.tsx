...
const Dashboardpage = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const response: ProgressResponse = await getDashboardData();
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6 mb-5">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="group relative bg-white/80 backdrop-blur-xl border border-slate-300/80 rounded-2xl shadow-xl shadow-slate-200/50 p-6 hover:shadow-2xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-2"
            >
              <div className="flex items-center justify-between">
                ...
                >
                  <stat.icon className="text-white w-5 h-5 " strokeWidth={2} />
                </div>
              </div>
                ...
                {[
                  ...(dashboardData.recentActivity.documents || []).map(
                    (doc) => ({
                      ...
                      timestamp: doc.lastAccessed as string,
                  ...
                  ...(dashboardData.recentActivity.quizzes || []).map(
                    (quiz) => ({
                      ...
                      timestamp: quiz.completedAt as string,
                      link: `/quiz/${quiz._id}`,
                      type: "quiz",
                    }),
                  ),
                  ...
                  .map((Activity, index) => (
                    <div
                      ...
                      className="group flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-200/60 hover:bg-white hover:border-slate-300/60 hover:shadow-md transition-all duration-300"
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {Activity.link && (
                          <Link
                            to={Activity.link}
                            className="ml-4 px-4 py-2 text-xs font-semibold text-emerald-600 hover:text-teal-600 hover:bg-emerald-50 rounded-lg transition-all duration-300 whitespace-nowrap"
                          >
                            View
                          </Link>
                        )}
