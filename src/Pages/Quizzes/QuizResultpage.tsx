...
const QuizResultpage = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      setLoading(true);
      try {
        const data = await getQuizResults(quizId as string);
        setResults(data);
  ...
  if (!results || !results.data || !results.data.quiz) {
    return (
      <div className="flex  items-center justify-center">
        <div className="text-center">
          <p className="text-slate-600 text-lg">Quiz result not founds.</p>
        </div>
      </div>
    );
  }

  const { quiz, detailedResults } = results.data;
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-300/25 p-8 mb-8">
        <div className="text-center space-y-6">
          <div className="inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 shadow-lg shadow-emerald-600/40">
            <Trophy className="w-7 h-7 text-emerald-500" strokeWidth={2} />
          </div>
          <div>
          ...
          <div className="flex items-center justify-center flex-wrap gap-4 pt-4">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-300 rounded-xl">
              <CheckCircle2
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-rose-50 border border-rose-300 rounded-xl">
      ...
      <div className="space-y-6">
        ...
        {detailedResults.map((result, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl p-6 shadow-lg shadow-slate-200/60"
            >
              <div className="flex itens-start justify-between gap-4 mb-3">
                <div className="flex-1">
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-50 border border-slate-200 rounded-lg mb-4">
                  ...
                  return (
                    <div
                      ...
                      className={`relative px-4 py-3 rounded-lg border-2 transition-all duration-300 ${
                        isCorrectOption
                          ? "bg-emerald-50 border-emerald-300 shadow-lg shadow-emerald-500/40"
                          : isWrongAnswer
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          {isCorrectOption && (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 border border-emerald-400 rounded-lg text-xs font-semibold text-emerald-700">
                              <CheckCircle2
                                className="w-3 h-3"
                                strokeWidth={2}
                              />
                          ...
                          {isWrongAnswer && (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-rose-100 border border-rose-400 rounded-lg text-xs font-semibold text-rose-700">
