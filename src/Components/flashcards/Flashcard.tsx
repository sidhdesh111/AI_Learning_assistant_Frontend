...
const Flashcard = (props: flascardPropsType) => {
  ...
  return (
    <div className="relative w-full h-72" style={{ perspective: "1000px" }}>
      <div
        className={`relative w-full h-full transition-transform duration-300 transform-gpu cursor-pointer`}
        style={{
      ...
      >
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                props.onToggleStar?.(props.flashcard._id);
              }}
            ...
            >
              <Star
                className="w-4 h-4"
                strokeWidth={2}
                fill={props.flashcard.isStarred ? "currentColor" : "none"}
              />
            </button>
          </div>
        ...
        {/* back of the card  */}
        <div
          className=" absolute inset-0 w-full h-full bg-linear-to-br from-emerald-500 to-teal-500 border-emerald-400/60 rounded-2xl shadow-xl shadow-emerald-400/30 p-8 flex flex-col justify-between"
          style={{
            backfaceVisibility: "hidden",
            WebkitBackfaceVisibility: "hidden",
